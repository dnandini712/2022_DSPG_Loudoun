...
sterlingPV <- read_csv("Property_Value.csv",skip=1,col_names=TRUE)
#subset_sterlingPV <- sterlingPV[c(92:99),c(1,2:3)]
...
```{r}
# Sample Pie Chart
slices.HOUSING <- c(73.9, 26.1)
lbls.HOUSING <- c("Owner-Occupied 73.9%", "Renter-Occupied 26.1%")
pie(slices.HOUSING, labels = lbls.HOUSING, main="Pie Chart of Home-owners vs Home-renters In Sterling,VA")

# 3D Exploded Pie Chart
library(plotrix)
slices.HOUSING <- c(73.9, 26.1)
lbls.HOUSING <- c("Owner-Occupied", "Renter-Occupied")
pie3D(slices.HOUSING,labels=lbls.HOUSING,explode=0.5,
   main="Pie Chart of Home-owners vs Home-renters In Sterling,VA") 
```
