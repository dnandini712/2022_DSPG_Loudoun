...
#public transportation within 45 minutes
```{r}
traveltimepub <- traveltime_map(appId = YourAppId,
                                apiKey = YourAPIKey,
                                location = c(39.009006,-77.4029155),
                                traveltime = 2700,
                                type="public_transport",
                                departure = "2022-07-09T08:00:00+01:00")
```
...
```{r}
m2 = mapview(traveltimepub,col.regions = c("grey"))
```
...
```{r}
mapfood <- read_excel("~/Downloads/DSPG-R-Training/2022_DSPG_Loudoun/shinyapp/data/Health and Social Services.xlsx", sheet = "Food")
mapfood$Longitude <- as.numeric(mapfood$Longitude)
mapfood$Latitude <- as.numeric(mapfood$Latitude)

mapclothe <-  read_excel("~/Downloads/DSPG-R-Training/2022_DSPG_Loudoun/shinyapp/data/Health and Social Services.xlsx", sheet = "Clothing")
mapclothe$Longitudes <- as.numeric(mapclothe$Longitude)
mapclothe$Latitudes <- as.numeric(mapclothe$Latitude)

maphealth <- merge(mapfood,mapclothe)
...
leaflet(data = maphealth) %>% addTiles() %>%
   addAwesomeMarkers(~Longitude, ~Latitude, popup = ~as.character(Location), icon=icons) %>%
  addAwesomeMarkers(~Longitudes, ~Latitudes, popup = ~as.character(`Organization name`), icon=icons2) 
...
smoothFactor = 0.2,
fillOpacity = 0.5)  %>% 
    addPolygons(data=traveltime20, color= "black",opacity = 1,weight=2,fillColor = "white", fillOpacity = .1) %>% addPolygons(data=traveltime10,color="green",opacity=1,weight=2,fillColor = "white",fillOpacity = .1) %>% addPolygons(data=traveltime45,color="blue",opacity = 1,weight = 2,fillColor = "white",fillOpacity = .1) %>%
  addMarkers( ~Longitude, ~Latitude, popup = ~as.character(Address), label = ~as.character(School), labelOptions = TRUE) %>% leaflet(data = foodpoints) %>% addTiles() %>% addMarkers(~Longitude, ~Latitude, popup = ~as.character(Location), label = ~as.character(Location),labelOptions = TRUE)
```
