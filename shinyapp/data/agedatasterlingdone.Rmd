...
library(dplyr)
library(tidyverse)
sterling <- read_excel("~/Downloads/DSPG-R-Training/2022_DSPG_Loudoun/shinyapp/data/sterlingdata.xlsx",skip=1,col_names=TRUE)
...
ages <- ggplot(subset_sterling,aes(x=age_cat,y=age_percent, fill=age_cat))+geom_col()+theme(axis.text.x=element_blank(), axis.title.x = element_blank(), axis.text.y = element_blank(),axis.ticks.y=element_blank(),axis.ticks.x=element_blank())+scale_x_discrete(limits=age_cat)+labs(title = "Age Group Distribution", caption= "Source: S0101 ACS 5-year data 2016-2020",y="Percent",)+guides(fill = guide_legend(title = "Age Group")) + geom_text(aes(label = age_percent, y = age_percent), size = 3, position = position_stack(vjust = 0.8))
```
...
plot_ly(subset_sterling,x=~Age, y=~Percent,type = "bar", color = ~Age, hoverinfo = "text",text = ~paste("Age:",Age,"<br>","Percent:",Percent,"%" )) %>% layout(title = "Age Distribution",xaxis = list(title=""))
