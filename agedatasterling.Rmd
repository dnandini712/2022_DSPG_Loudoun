...
subset_sterling
sterling$Estimate...10[6:18]
age_estimate <- sterling$Estimate...10[6:18]
as.numeric(gsub(",","",age_estimate))
age_estimate <-as.numeric(gsub(",","",age_estimate))
age_cat <- (subset_sterling$Label)
#this is how to subset the data
#turn categories into factors 
ggplot(subset_sterling,aes(x=age_cat,y=age_estimate, fill=age_cat))+geom_col()+theme(axis.text.x=element_text(angle=22.5, size=7.5, face="bold"))+scale_x_discrete(limits=age_cat)+labs(title="Age Demographics of Sterling, VA",subtitle="2016-2020",caption= "Source: ACS data 2016-2020",x="Age",y="Estimated Population") + coord_polar()
